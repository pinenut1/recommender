...
const SHIKIGAMI_LIST = [
  ...
  "祸津神","神无月","龙珏","sp岚","太奶","千姬","云外镜","ssr蛇",
  "sp小白","泷","猫川","月读","缘结神","sp鹿丸","雨女","sp猫","老头",
...
function evaluateStrategy(strategy) {
  ...
  const hasBan = currentBan && strategy.my.includes(currentBan);
...
function updateRecommendations() {
  ...
  evaluated.forEach(s => {
    ...
    if (s.hasBan) tags.push('<span class="tag-ban">包含 ban</span>');

    const myLine = s.my.map(m =>
      m === currentBan
        ? `<span class="ban-shiki">${m}</span>`
        : m
    ).join(" → ");

    const enemyLine = s.enemy.map(e =>
      enemyPicks.includes(e)
        ? `<span class="enemy-hit">${e}</span>`
        : e
    ).join(" → ");
    ...
    div.innerHTML = `
      ...
      <p class="enemy-line">敌方阵容：${enemyLine}</p>
